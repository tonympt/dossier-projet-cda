...
| **users** | `GET /users`, `GET /users/:id`, `PATCH /users/:id/role` | Admin |
...
### Étape 1 — Panier
Client-side ; `POST /cart/batch-details` (public) enrichit les items (prix, stock). Aucun écrit en base.
...
**Le problème.** Que se passe-t-il si le client paie (argent débité) mais que le **webhook n'arrive
jamais** (serveur momentanément down, réseau) ? La commande reste bloquée en `PROCESSING`. Cas
**critique** : il ne faut **surtout pas** l'annuler sans vérifier, sinon on garde l'argent sans livrer.
...
**La réconciliation proprement dite** : on interroge Stripe pour connaître l'état **réel**.
...
**Robustesse à connaître** : seuil 30 min **≥ timeout du checkout** (ne pas nettoyer une commande
pendant que le client paie encore) ; mise à jour BDD et appel Stripe séparés (une erreur Stripe ne
bloque pas le nettoyage DB) ; un try/catch **par commande** (une erreur n'interrompt pas les autres).
...
## 9. Mémo express (à relire juste avant l'oral)
